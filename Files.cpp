	...
	modelData getVertexData(const char * filename){
		...
		while(1){
			...
			if(!strcmp(keyword, "element")){
				...
				if(!strcmp(element_name, "vertex")){
					...
					size_t vertexSize = nVerts*3*2;
					out.vertexBytes = nVerts*3*2*sizeof(out.vertexData[0]);
					out.vertexData = new GLfloat[vertexSize];//3 for the position, 3 for the color, 3 for the normal
				}
				else
		...
		for(size_t v = 0; v < nVerts; v++){
			...
			for(int c = 0; c < 3; c++){
				float color;
				fscanf(file, "%f", &color);
				out.vertexData[nVerts*3+3*v+c] = color/255.0;
			}
		}
